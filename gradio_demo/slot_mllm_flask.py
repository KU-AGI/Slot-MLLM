...
class LLMService:
    def __init__(self, args) -> None:
        ...
        self.image_tokenizer = SlotQFormerModel.from_pretrained("zheedong/Slot_Q-Former")
        ...
        model = SlotMLLMInferenceWrapper(llm_model, self.image_tokenizer, self.text_tokenizer, self.image_transform, special_tokens)
            
        self.model = model
        print(model.get_memory_footprint())
        self.llm_device = args.llm_device
        self.tokenizer_device = args.tokenizer_device
        self.boi_token_id = self.text_tokenizer(BOI_TOKEN, add_special_tokens=False).input_ids[0]
        self.eoi_token_id = self.text_tokenizer(EOI_TOKEN, add_special_tokens=False).input_ids[0]
        print('Init Done...')
...
def generate():
    ...
    if len(image_list) > 0:
        ...
        if len(images_tensor_list) > 0:
            ...
            images_ids_1 = service.image_tokenizer.forward_stage_1(images_tensor).cpu()
        ...
        input_text = service.text_tokenizer.bos_token + input_text + text_list[-1]
    ...
    print(input_text)
    input_ids = service.text_tokenizer(input_text, add_special_tokens=False, return_tensors='pt').input_ids
    ...
    generate_text = service.image_tokenizer.decode(generate_ids, skip_special_tokens=False)
