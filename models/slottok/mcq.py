...
class VectorQuantizerM(nn.Module):
    def __init__(
        ...
        # TODO : hardcode the projection layer
        encoder_embed_dim = 768
        self.quant_proj = AttnProjection(encoder_embed_dim, vocab_width, encoder_embed_dim // vocab_width)
        self.post_quant_proj = AttnProjection(vocab_width, encoder_embed_dim, encoder_embed_dim // vocab_width)

        self.fc_norm = nn.LayerNorm(encoder_embed_dim, eps=1e-6)
        self.projection = nn.Linear(encoder_embed_dim, encoder_embed_dim)
    ...
    def encode(self, features):
        img_tokens = self.quant_proj(features)
        img_indices = self.f_to_idx(img_tokens)
        img_tokens = self.idx_to_f(img_indices)
        img_tokens = self.post_quant_proj(img_tokens)
        features = img_tokens.mean(dim=1)
        features = self.projection(self.fc_norm(features))
        return features

    def forward_w_proj(self, features):
        img_tokens = self.quant_proj(features)
        img_tokens, vq_loss, entropy_loss, usages = self.forward(img_tokens)
        img_indices = self.f_to_idx(img_tokens)
        img_tokens = self.post_quant_proj(img_tokens)
        return img_tokens, img_indices, (0.2 * vq_loss + 1.0 * entropy_loss)

    def idx_to_f_w_proj(self, indices):
        features = self.idx_to_f(indices)
