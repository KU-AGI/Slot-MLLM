...
from gradio_demo.conversation import conv_seed_vicuna, conv_seed_llama2
...
elif args.model_type == 'slot-mllm-8b':
    conv_seed_llama = conv_seed_vicuna
...
def http_bot(dialog_state, input_state, temperature, top_p, max_new_tokens, num_beams, max_turns, force_image_gen, request: gr.Request):
    logger.info(f"http_bot. ip: {request.client.host}")
    logger.debug(f"input_state: {input_state}")
    logger.debug(f"dialog_state messages count: {len(dialog_state.messages)}")
    ...
    try:
        if len(dialog_state.messages) == 0 or dialog_state.messages[-1]['role'] != dialog_state.roles[0] or len(
            ...
            logger.debug("No valid message to process")
            return (dialog_state, input_state, dialog_state.to_gradio_chatbot()) + (no_change_btn, ) * 4

        if len(dialog_state.messages) > max_turns * 2:
            logger.warning(f"History exceeds maximum rounds: {len(dialog_state.messages)} > {max_turns * 2}")
        ...
        logger.info(f"Request address: {args.request_address}")
        
        # HTTP 요청 타임아웃 설정
        response = requests.request(
            method="POST", 
            url=args.request_address, 
            headers=headers, 
            json=payload,
        ...
        if response.status_code != 200:
            ...
            output_state = init_input_state()
            output_state['text'] = f'Error: Backend server returned status {response.status_code}. Please check if the backend is running.'
            dialog_state.messages.append({'role': dialog_state.roles[1], 'message': output_state})
            input_state = init_input_state()
            return (dialog_state, input_state, dialog_state.to_gradio_chatbot()) + (disable_btn, ) * 3 + (enable_btn, )
        
        results = response.json()
        logger.info(f"Response results: {results}")

        output_state = init_input_state()
        image_dir = get_conv_image_dir()
        output_state['text'] = results['text']

        for i, image_base64 in enumerate(results['images']):
            logger.debug(f"Processing image {i+1}/{len(results['images'])}")
            if image_base64 == '':
                image_path = ''
                logger.debug(f"Empty image at index {i}")
            else:
                try:
        ...
        output_state = init_input_state()
        output_state['text'] = 'Error: Request timeout. The backend server is taking too long to respond.'
        dialog_state.messages.append({'role': dialog_state.roles[1], 'message': output_state})
        input_state = init_input_state()
        return (dialog_state, input_state, dialog_state.to_gradio_chatbot()) + (disable_btn, ) * 3 + (enable_btn, )
        ...
        output_state = init_input_state()
        output_state['text'] = 'Error: Cannot connect to backend server. Please check if the backend is running at the correct address.'
        dialog_state.messages.append({'role': dialog_state.roles[1], 'message': output_state})
        input_state = init_input_state()
        return (dialog_state, input_state, dialog_state.to_gradio_chatbot()) + (disable_btn, ) * 3 + (enable_btn, )
        
    except Exception as e:
        logger.error(f"Unexpected error in http_bot: {e}")
        logger.error(f"Traceback: {traceback.format_exc()}")
        output_state = init_input_state()
        output_state['text'] = f'Error: An unexpected error occurred: {str(e)}'
        dialog_state.messages.append({'role': dialog_state.roles[1], 'message': output_state})
        input_state = init_input_state()
        return (dialog_state, input_state, dialog_state.to_gradio_chatbot()) + (disable_btn, ) * 3 + (enable_btn, )
...
if __name__ == '__main__':
    ...
    demo.launch(server_name=args.server_name, server_port=args.server_port, enable_queue=True, share=False)
