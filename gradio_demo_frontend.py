...
from gradio_demo.utils import build_logger
from gradio_demo.conversation import conv_seed_vicuna, conv_seed_llama2, conv_qwen_2_5
...
# ─── 새로 추가 ─────────────────────────────────────────────────────────
MODEL2ADDR = {           # 드롭다운 값 ➜ backend REST 주소
    "SEED-LLaMA-8B":  "http://127.0.0.1:7880/generate",
    "SEED-LLaMA-14B": "http://127.0.0.1:7881/generate",  # 예: LLaMA-2 서버 포트
    "Slot-MLLM-8B":   "http://127.0.0.1:7882/generate",   # 예: slot 서버 포트
    "Slot-MLLM-14B":   "http://127.0.0.1:7883/generate",   # 예: slot 14b 서버 포트
}

MODEL2CONV = {
    # UI 드롭다운 값          # 대응하는 시드(프롬프트 템플릿)
    "SEED-LLaMA-8B": conv_seed_vicuna,   # Vicuna-style
    "SEED-LLaMA-14B": conv_seed_llama2,  # LLaMA-2-style (필요하면 dropdown에 추가)
    "Slot-MLLM-8B":  conv_seed_vicuna,  # Slot-MLLM도 Vicuna-style
    "Slot-MLLM-14B": conv_qwen_2_5,  # TODO: Change for Qwne 2.5 Style
}

def get_conv_seed(model_choice: str = "Slot-MLLM-8B"):
    """드롭다운 값 → 시드 객체(.copy()까지) 반환"""
    return MODEL2CONV.get(model_choice, conv_seed_vicuna).copy()
...
class Arguments:
    ...
    # model_type: Optional[str] = field(default='seed-llama-14b', metadata={"help": "choice: [seed-llama-8b, seed-llama-14b, slot-mllm-8b]"})
...
def clear_history(model_choice, request: gr.Request = None):
    if request:
        logger.info(f"clear_history. ip: {request.client.host}")
    dialog_state = get_conv_seed(model_choice)
...
def http_bot(dialog_state, input_state, temperature, top_p, max_new_tokens, num_beams, max_turns, force_image_gen, model_choice, request: gr.Request):
    logger.info(f"http_bot. ip: {request.client.host}")
    ...
    if len(dialog_state.messages) > max_turns * 2:
        ...
        dialog_state.messages.append({'role': dialog_state.roles[1], 'message': output_state})
        input_state = init_input_state()
        return (dialog_state, input_state, dialog_state.to_gradio_chatbot()) + (disable_btn, ) + (enable_btn, )
    ...
    request_address = MODEL2ADDR[model_choice]
...
def load_demo(model_choice, request: gr.Request):
    if request:
    ...
    with gr.Blocks(css=css) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Row():  # ← 이미지·텍스트 입력 영역 바로 아래
                    model_selector = gr.Dropdown(
                        choices=list(MODEL2ADDR.keys()),
                        value="Slot-MLLM-14B",  # 기본값
                        label="🔀 Backend Model",
                        scale=1  # 텍스트 상자 폭과 맞춤
                    )
        ...
        regenerate_btn.click(regenerate, [dialog_state], [dialog_state, chatbot] + btn_list).then(
            http_bot, [dialog_state, input_state, temperature, top_p, max_new_tokens, num_beams, max_turns, force_img_gen, model_selector],
        ...
        submit_btn.click(
            add_image, [dialog_state, input_state, image], [dialog_state, input_state, image, chatbot] + btn_list).then(
                ...
                [dialog_state, input_state, text, chatbot, regenerate_btn, clear_btn]).then(
                    http_bot, [dialog_state, input_state, temperature, top_p, max_new_tokens, num_beams, max_turns, force_img_gen, model_selector],
                    [dialog_state, input_state, chatbot] + btn_list)
        clear_btn.click(clear_history, [model_selector], [dialog_state, input_state, chatbot] + btn_list)
        ...
        demo.load(load_demo, [model_selector], [dialog_state, input_state])

    demo.launch(server_name=args.server_name, server_port=args.server_port, enable_queue=True, share=True)
